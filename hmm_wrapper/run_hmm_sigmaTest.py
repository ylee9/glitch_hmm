...
def setup_hmm(par, tim, config):
    ...
    if 'tn' in config:
        ...
        elif 'sigma_min' in config['tn'] or 'sigma_max' in config['tn'] or 'sigma_num' in config['tn']:
            print('Generating a list of sigma')
            try:
                sigma = np.logspace(np.log10(float(config['tn']['sigma_min'])), np.log10(float(config['tn']['sigma_max'])), num=int(config['tn']['sigma_num']), base=10.0)
            except KeyError:
                print('Need to specify all three of sigma_min, sigma_max and sigma_num')
                sigma = None
                print('Sigma is not defined!!')
    else:
...
def do_psr(hmm, sigma, config, extra_matlab_cmd=None, ul=False):
    ...
    glitch_model = config['models']['glitch_model']
    out_prefix = config['out']['out_prefix']
    output_all = config['out']['output_all']
    working_prefix = config['matlab']['working_prefix'] if 'working_prefix' in config['matlab'] else None


    matlab_cmd = "global f_fiducial fd_fiducial fdd_fiducial sigma kappa_per_toa glitch_model output_all;"
    ...
    if isinstance(sigma, float):
        ...
        matlab_cmd += f"sigma=["+ " ".join([str(x) for x in sigma]) + "];"
    matlab_cmd += f"glitch_model = '{glitch_model}';"
    matlab_cmd += f"output_all = '{output_all}';"
