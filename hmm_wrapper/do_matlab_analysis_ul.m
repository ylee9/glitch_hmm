...
for i = 0:(num_realisations-1)
    ...
    [path,gamma,alpha,beta,evidence0,s, residuals] = HMM_Pulse_3dcol(zs, freqs', fdots',[]);
    %[path,gamma,alpha,beta,evidence,s, residuals] = HMM_Pulse_3dcol(zs, freqs', fdots',[glitch]);
    evidence = zeros(3, length(zs)+1);
    [~,evidence(1,:)] = HMM_Pulse_3dcol_seq(zs,freqs',fdots',[glitch-1],alpha,evidence0,s,glitch-1);
    [~,evidence(2,:)] = HMM_Pulse_3dcol_seq(zs,freqs',fdots',[glitch],alpha,evidence0,s,glitch);
    [~,evidence(3,:)] = HMM_Pulse_3dcol_seq(zs,freqs',fdots',[glitch+1],alpha,evidence0,s,glitch+1);
