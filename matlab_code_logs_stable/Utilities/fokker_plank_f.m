...
red_idx = 4.35219;
red_amp = 10^(-8.99626);
phase_cov = f_fiducial^2*red_amp^2/(12*pi^2)*(pi*1e7)^(4-red_idx)/gamma(red_idx)*z^(red_idx-1);
V = [phase_cov/z^2 phase_cov/z^2*(1e-17*864000^0.5); phase_cov/z^2*(1e-17*864000^0.5) (1e-17*864000^0.5)^2];
%}
...
q = q - logsumexp(q(:));
r=[];
mu=round(-z*y'/df); % mean forward displacement in bins
mu=mu(K+(1:2*K+1));
if gi
	%r - pdf of glitch
	gb=round(z*y'/df)-1;
	gb0=gb+N+m; %make zero fdot  end at  (N-1)-th f bin (offset by m to remove any effect of convolution)
	ind=repmat(1:N*2+m,Kq,1); %add N zeros at the end
	n=find(ind<=gb0 & ind>gb0-1e4);
    %n=find(ind<=gb0-100 & ind>gb0-1e4); %TODO remove
	r0=zeros(size(ind));
	r0(n)=1;
	%convolve with q TODO logs?
